...
def predict_gw(gw_dir):
    ...
    # Moving Averages for Recent Form - 3 and 5 game averages, including 'bonus'
    for df, features in [
        (midfielders, ['goals', 'assists', 'form', 'xG', 'xA', 'ict_index', 'bonus', 'key_passes', 'shots']),
    ...
    # Weighted Averages for last 5 games (EMA)
    for df, features in [
        (midfielders, ['goals', 'assists', 'form', 'xG', 'xA', 'ict_index', 'bonus']),
    ...
    # Comparative Metrics (Midfielders, Forwards)
    for df in [midfielders, forwards]:
        df['points_per_minute_delta'] = (
            (df['total_points'] / df['minutes'].replace(0, 1)) - df['last_season_points_per_minute']
        )

    # Defensive Interaction Features (Defenders)
    defenders['clean_sheets_form_interaction'] = (
        defenders['clean_sheets_3g_avg'] * defenders['form_3g_avg']
    )

    # Momentum Indicators based on 3-game averages
    for df, features in [
        (midfielders, ['form', 'xG', 'xA']),
        (forwards, ['form', 'xG', 'xA']),
        (defenders, ['expected_goals_conceded', 'form']),
        (goalkeepers, ['expected_goals_conceded', 'saves'])
    ]:
        for feature in features:
            if feature in df.columns:
    ...
    # Per-90 Stats (All Positions)
    for df, feature_list in [
        ...
        for feature in feature_list:
            ...
            df[f'{feature}_per_90'] = np.where(
                ...
                df[f'cumulative_{feature}'] / (df['cumulative_minutes'] / 90),
                0
            )
    ...
    # Rolling Averages for Fixture Difficulties
    for df in [goalkeepers, defenders, midfielders, forwards]:
