...
def adjust_predictions_by_availability(prediction_csv):
    ...
    for idx, row in prediction_df.iterrows():
        player_name = str(row['name'])  # Convert player_name to string to avoid similar issues
...
GOALKEEPER_FEATURES = [
        ...
        'last_season_clean_sheet_probability', 'saves_clean_sheets_3g', 
...
def predict_gw(gw_dir):
    ...
    apply_trained_model(rf_gk_model, GOALKEEPER_FEATURES, 'prediction_data/2024-25/%s/gk.csv'%(gw_dir))
    apply_trained_model(rf_def_model, DEFENDER_FEATURES, 'prediction_data/2024-25/%s/def.csv'%(gw_dir))
    apply_trained_model(rf_mid_model, MIDFIELDER_FEATURES, 'prediction_data/2024-25/%s/mid.csv'%(gw_dir))
    apply_trained_model(rf_fwd_model, FORWARD_FEATURES, 'prediction_data/2024-25/%s/fwd.csv'%(gw_dir))


    # Apply adjustments to each prediction CSV file
    adjust_predictions_by_availability(f'prediction_data/2024-25/{gw_dir}/gk.csv')
    adjust_predictions_by_availability(f'prediction_data/2024-25/{gw_dir}/def.csv')
    adjust_predictions_by_availability(f'prediction_data/2024-25/{gw_dir}/mid.csv')
    adjust_predictions_by_availability(f'prediction_data/2024-25/{gw_dir}/fwd.csv')
