            ...
            if os.path.exists(position_dir):
                for player_file in os.listdir(position_dir):
                    ...
                    if season_index == len(seasons) - 1:
                        # Handle last season case by searching in fixtures_for_each_team_dir
                        team_name = df['team'].iloc[0]  # Get the team name from the player's team column
                        expected_filename = f"{team_name}_{season}.csv"

                        # Fuzzy match the filename in fixtures_for_each_team_dir
                        available_files = os.listdir(fixtures_for_each_team_dir)
                        matched_file, _ = process.extractOne(expected_filename, available_files)
                        
                        if matched_file:
                            fixture_path = os.path.join(fixtures_for_each_team_dir, matched_file)
                            fixture_df = pd.read_csv(fixture_path)

                            # Find rows with missing 'Opponent Score' to get future fixtures
                            next_fixture_rows = fixture_df[pd.to_numeric(fixture_df['Team Score'], errors='coerce').isna()]

                            # Add fixture difficulty for the last row
                            if not next_fixture_rows.empty:
                                next_fixture_row = next_fixture_rows.iloc[0]
                                df.loc[last_row_index, 'next_team'] = df['team'].iloc[last_row_index]
                                df.loc[last_row_index, 'next_opponent_team'] = next_fixture_row['Opponent']
                                df.loc[last_row_index, 'next_was_home'] = True if next_fixture_row['Location'] == 'Home' else False

                            # Add fixture difficulty for one additional row if there are 37 rows
                            if num_rows == 37 and len(next_fixture_rows) > 1:
                                additional_row_index = last_row_index + 1
                                next_fixture_row = next_fixture_rows.iloc[1]
                                df = pd.concat([df, pd.DataFrame([{}])], ignore_index=True)
                                df.loc[additional_row_index, 'team'] = df['team'].iloc[last_row_index]
                                df.loc[additional_row_index, 'next_team'] = df['team'].iloc[last_row_index]
                                df.loc[additional_row_index, 'next_opponent_team'] = next_fixture_row['Opponent']
                                df.loc[additional_row_index, 'next_was_home'] = True if next_fixture_row['Location'] == 'Home' else False

                            # Add fixture difficulties for two additional rows if there are 36 or fewer rows
                            elif num_rows <= 36 and len(next_fixture_rows) > 2:
                                for i in range(2):
                                    additional_row_index = last_row_index + 1 + i
                                    next_fixture_row = next_fixture_rows.iloc[i + 1]
                                    df = pd.concat([df, pd.DataFrame([{}])], ignore_index=True)
                                    df.loc[additional_row_index, 'team'] = df['team'].iloc[last_row_index]
                                    df.loc[additional_row_index, 'next_team'] = df['team'].iloc[last_row_index]
                                    df.loc[additional_row_index, 'next_opponent_team'] = next_fixture_row['Opponent']
                                ...
                                print(f"No valid upcoming fixtures found for {team_name} in {fixture_path}")

                    elif season_index < len(seasons) - 1:
                        # Normal processing for other seasons
                    ...
                    # Apply specific fixture difficulty calculations
                    ...
                    # Apply holistic fixture difficulty calculations
